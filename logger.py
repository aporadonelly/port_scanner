...
def setup_logger(name="port_scanner", level=logging.INFO): #name can be anything
    """Defines a function called setup_logger that accepts name and logging level"""  

    logger = logging.getLogger(name)
    if not logger.handlers:
        # Console handler
        stream_handler = logging.StreamHandler() #creates a handler  that outputs logs to the console
